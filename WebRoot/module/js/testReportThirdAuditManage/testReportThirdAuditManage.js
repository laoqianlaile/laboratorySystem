...
function initData(){
	...
	//		width : '10px',// 宽度
		...
		//	width : '10px',// 宽度
			...
			width : '10%',// 宽度
			formatter : function(value, row, index) {
					return	"<button type='button' class='btn btn-success glyphicon glyphicon-ok-sign' onclick='thirdAuditPass(\""+row.ID+"\")' style='margin-right:10px'>"+
				        	"</button>"+
				        	"<button type='button' class='btn btn-warning glyphicon glyphicon-remove' onclick='thirdAuditReject(\""+row.ID+"\")'>"
				        	"</button>"	
			}
		}]
	});
