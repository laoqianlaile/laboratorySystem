...
// 人员信息注册方法
function add() {
	if (upperCase() && passwordCase() && companyCase() && linkmanCase()
			&& mobileCase() && fixedCase() && check() && emailCase() && faxCase()) {
		var parame = {};
...
function representativeCase() {
	...
	if (representative == "") {
		$('#representativePrompt').html("*法人不能为空");
		return false;
	}
	return true;
}
...
function linkmanCase() {
	var linkman = $("#linkman").val();
	if (linkman == "") {
		$("#linkmanPrompt").html("*代理人不能为空")
		return false;
	}
	return true;
}
function linkmanInput() {
	$("#linkmanPrompt").html("");
}
